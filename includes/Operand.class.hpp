...
public:
    ...
    void                setValue( T value ) { this->_value = value; return; }; // Value setter
    ...
    IOperand const *    createInt8( std::string const & value ) const { Operand<T> * _int8 = new Operand<T>; _int8->setValue(static_cast<int8_t>(atoi(value.c_str()))); _int8->setType(INT8); _int8->setString(value); return _int8; }; // Create int8 operand
    IOperand const *    createInt16( std::string const & value ) const { Operand<T> * _int16 = new Operand<T>; _int16->setValue(static_cast<int16_t>(atoi(value.c_str()))); _int16->setType(INT16); _int16->setString(value); return _int16; }; // Create int16 operand
    IOperand const *    createInt32( std::string const & value ) const { Operand<T> * _int32 = new Operand<T>; _int32->setValue(static_cast<int32_t>(atoi(value.c_str()))); _int32->setType(INT32); _int32->setString(value); return _int32; }; // Create int32 operand
    IOperand const *    createFloat( std::string const & value ) const {  Operand<T> * _float = new Operand<T>; _float->setValue(static_cast<float>(atof(value.c_str()))); _float->setType(FLOAT); _float->setString(value); return _float; }; // Create float operand
    IOperand const *    createDouble( std::string const & value ) const {  Operand<T> * _double = new Operand<T>; _double->setValue(static_cast<double>(atof(value.c_str()))); _double->setType(DOUBLE); _double->setString(value); return _double; }; // Create double operand
